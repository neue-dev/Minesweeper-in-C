 ...
 * @ Modified time: 2024-03-31 01:40:37
 * @ Description:
 * 
...
int Stats_update(Game *this) {
	...
	for(i = 0; i < this->field.dHeight; i++) {
		for(j = 0; j < this->field.dWidth; j++) {
			sGameGridData[i][j] = Grid_getBit(this->field.pMineGrid, j, i) ? 'X' : '.';		
		}
		sGameGridData[i][j] = '\n';
	}
	...
	for(i++; i < nProfileDataLength + this->field.dHeight + 1; i++) {
		sProfileNewDataArray[i] = String_create(sGameGridData[i - nProfileDataLength]);
