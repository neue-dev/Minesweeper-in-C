...
#define THREAD_MAX_COUNT 8                // Maximum number of threads
#define THREAD_TIMEOUT (long) 1000 / 60   // Length of the execution cycle (in ms) per thread
...
*/
Thread *Thread_init(Thread *this, HandleMutex hStateMutex, HandleMutex hDataMutex, ParamFunc pCallback, ParamObj pArgs) {
 ...
 * @return  { Thread * }        A pointer to the initialized thread object.
*/
Thread *Thread_create(HandleMutex hStateMutex, HandleMutex hDataMutex, ParamFunc pCallback, ParamObj pArgs) {
  return Thread_init(Thread_new(), hStateMutex, hDataMutex, pCallback, pArgs);
}
...
*/
int ThreadPool_createThread(ThreadPool *this) {
  Thread *pThread;
  ...
  pThread = Thread_create(this->hStateMutex, this->hDataMutex, NULL, NULL);
